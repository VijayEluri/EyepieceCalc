...
/**
 * <p>Database adapter class for Telescopes. Handles retrieving
 * and storing Telescope model objects and cursors from database.</p>
 * 
...
public class TelescopeAdapter {
	...
	public static final int APERTURE_COLUMN		= 3;
	
	private static final String[] resultColumns = { KEY_ID, KEY_NAME, KEY_FOCAL_LENGTH, KEY_APERTURE };
	...
	private TelescopeDbHelper dbHelper;

	public TelescopeAdapter(Context _context) {
		context = _context;
		dbHelper = new TelescopeDbHelper(context, DATABASE_NAME, null,
				DATABASE_VERSION);
	}

	public TelescopeAdapter open() throws SQLException {
     ...
     * use the values to populate an instance of Telescope
     * 
	 * @param _rowIndex
	 * @return
	 */
	public Telescope getEntry(long _rowIndex) {
		Telescope objectInstance = null;
		...
		if ( resultSet.moveToFirst() ) {
			objectInstance = new Telescope( resultSet.getLong( 0 ), 
											resultSet.getString( NAME_COLUMN ),
											resultSet.getInt( FOCAL_LENGTH_COLUMN ),
											resultSet.getInt( APERTURE_COLUMN ) );
		}
	 ...
	 * @return
	 */
	public boolean updateEntry(long _rowIndex, Telescope _myObject) {
		ContentValues newValues = new ContentValues();
	...
	private static class TelescopeDbHelper extends SQLiteOpenHelper {

		public TelescopeDbHelper(Context context, String name, CursorFactory factory,
				int version) {
		...
		public void onUpgrade(SQLiteDatabase _db, int _oldVersion,
			...
			Log.w("TelescopeDbHelper", "Upgrading from version " + _oldVersion
					+ " to " + _newVersion
